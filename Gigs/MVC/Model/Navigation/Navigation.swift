...
//        leftBtn.addTarget(self, action: #selector(leftBtnTapped), for: .touchUpInside)
        ...
        if rightButtonImageName.count != 0 {
            ...
            //   rightBtn.addTarget(self, action: #selector(DUProfileViewController.rightBtnTapped), for: .touchUpInside)
    ...
    private func setNavigationBarProperties(viewController : UIViewController) {
        ...
        viewController.navigationController?.navigationBar.barTintColor = HELPER.hexStringToUIColor(hex: APP_COLOR)
    }
